...
#define NEWHASH_SCR(i)                         \
{                                             \
w[i] =  NEWHASH_F4(w[i -  2]) + w[i -  7]  \
+ NEWHASH_F3(w[i - 15]) + w[i - 16]; \
}

unsigned int NEWHASH_h0[8] =
{ 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19 };

unsigned int NEWHASH_k[64] =
...
void NEWHASH_transf(NEWHASH_ctx *ctx, const unsigned char *message,
                   unsigned int block_nb)
{
    ...
    for (i = 0; i < (int)block_nb; i++) {
        ...
        for (j = 0; j < 64; j++) {
            t1 = wv[7] + NEWHASH_F2(wv[4]) + CH(wv[4], wv[5], wv[6])
            + NEWHASH_k[j] + w[j];
            t2 = NEWHASH_F1(wv[0]) + MAJ(wv[0], wv[1], wv[2]);
            wv[7] = wv[6];
            wv[6] = wv[5];
            wv[5] = wv[4];
            wv[4] = wv[3] + t1;
...
void NEWHASH(const unsigned char *message, unsigned int len, unsigned char *digest)
{
    NEWHASH_ctx ctx;

    NEWHASH_init(&ctx);
    NEWHASH_update(&ctx, message, len);
    NEWHASH_final(&ctx, digest);
}
                   ...
                   unsigned int len)
{
    ...
    tmp_len = NEWHASH_BLOCK_SIZE - ctx->len;
    rem_len = len < tmp_len ? len : tmp_len;

    memcpy(&ctx->block[ctx->len], message, rem_len);

    if (ctx->len + len < NEWHASH_BLOCK_SIZE) {
        ctx->len += len;
        return;
    }

    new_len = len - rem_len;
    block_nb = new_len / NEWHASH_BLOCK_SIZE;

    shifted_message = message + rem_len;

    NEWHASH_transf(ctx, ctx->block, 1);
    NEWHASH_transf(ctx, shifted_message, block_nb);

    rem_len = new_len % NEWHASH_BLOCK_SIZE;
...
void NEWHASH_final(NEWHASH_ctx *ctx, unsigned char *digest)
{
    ...
    block_nb = (1 + ((NEWHASH_BLOCK_SIZE - 9)
                     < (ctx->len % NEWHASH_BLOCK_SIZE)));
    ...
    NEWHASH_transf(ctx, ctx->block, block_nb);
...
unsigned char * HMAC_NEWHASH(const char * msg, const char * key)
{
    ...
    unsigned char * HMAC_NEWHASH = (unsigned char *)malloc(32 * sizeof(unsigned char));
    ...
    NEWHASH(Key0_ipad_concat_text, blocksize + (unsigned int)strlen(msg), Key0_ipad_concat_text_digest);
    ...
    NEWHASH(Key0_opad_concat_prev, blocksize + 32, HMAC_NEWHASH);
    return HMAC_NEWHASH;
}
...
int main()
{
    ...
    result = HMAC_NEWHASH("Sample #007", "Hey...How are you?");
